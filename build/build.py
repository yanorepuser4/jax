...
def write_bazelrc(*, remote_build,
  ...
  with open("../.jax_configure.bazelrc", "w") as f:
    ...
    if enable_cuda:
      ...
      if cuda_version:
        f.write("build --action_env HERMETIC_CUDA_VERSION=\"{cuda_version}\"\n"
                .format(cuda_version=cuda_version))
      if cudnn_version:
        f.write("build --action_env HERMETIC_CUDNN_VERSION=\"{cudnn_version}\"\n"
                .format(cudnn_version=cudnn_version))
      if cuda_compute_capabilities:
        f.write(
          f'build:cuda --action_env HERMETIC_CUDA_COMPUTE_CAPABILITIES="{cuda_compute_capabilities}"\n')
    if enable_rocm:
