...
LogicalResult scf_for_rule(RewriteContext &ctx, Operation &op,
  ...
  for (auto [in_layout, yield_layout, out_layout, result] :
    ...
    if (auto vty = dyn_cast<VectorType>(result.getType())) {
      ...
      if (!in_layout->generalizes(*out_layout, vty.getShape(),
                                  ctx.target_shape)) {
        return op.emitOpError(
                   "Not implemented: for loop input layout can not generalzie "
                   "output layout ")
               << out_idx;
      }
      if (!yield_layout->generalizes(*out_layout, vty.getShape(),
                                     ctx.target_shape)) {
        return op.emitOpError(
                   "Not implemented: for loop yield layout can not generalzie "
                   "output layout ")
               << out_idx;
      }
    } else {
  ...
  for (auto [in_layout, cond_layout, yield_layout, out_layout, result] :
    ...
    if (auto vty = dyn_cast<VectorType>(result.getType())) {
      ...
      if (!in_layout->generalizes(*out_layout, vty.getShape(),
                                  ctx.target_shape)) {
        return op.emitOpError(
                   "Not implemented: while loop input layout can not "
                   "generalzie output layout ")
               << out_idx;
      }
      if (!cond_layout->generalizes(*out_layout, vty.getShape(),
                                    ctx.target_shape)) {
        return op.emitOpError(
                   "Not implemented: while loop condition layout can not "
                   "generalzie output layout ")
               << out_idx;
      }
      if (!yield_layout->generalizes(*out_layout, vty.getShape(),
                                     ctx.target_shape)) {
        return op.emitOpError(
                   "Not implemented: while loop yield layout can not "
                   "generalzie output layout ")
               << out_idx;
      }
    } else {
...
LogicalResult scf_if_rule(RewriteContext &ctx, Operation &op,
  ...
  // Check if yield layout from then/else branch can generalize result layout.
  int out_idx = 0;
  for (auto [then_layout, else_layout, result_layout, result] :
    ...
    if (auto vty = dyn_cast<VectorType>(result.getType())) {
      ...
      if (!then_layout->generalizes(*result_layout, vty.getShape(),
                                    ctx.target_shape)) {
        return op.emitOpError(
                   "Not implemented: yield layout from then branch can not "
                   "generalzie result layout ")
               << out_idx;
      }
      if (!else_layout->generalizes(*result_layout, vty.getShape(),
                                    ctx.target_shape)) {
        return op.emitOpError(
                   "Not implemented: yield layout from else branch can not "
                   "generalzie result layout ")
               << out_idx;
      }
    } else {
